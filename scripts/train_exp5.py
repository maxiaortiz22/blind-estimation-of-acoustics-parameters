...
def create_dataset(band, db_name, sample_frac=1.0, random_state=None):
    ...
    for partition in partitions:
        #Leo la base de datos:
        aux_df = pd.read_pickle(f'cache/{db_name}/{partition}')
        #Filtro por la banda y solo me quedo con las RIRs reales:
        db = db.append(aux_df.loc[aux_df.banda == band], ignore_index=True)
        db = db[db.ReverbedAudio.str.lower().str.contains("original")]
        db = db[~db.ReverbedAudio.str.lower().str.contains("sintetica")]
        bar.next()
...
if __name__ == '__main__':
    ...
    exp_num = 5 #Número del experimento
    ...
    new_db_name = 'exp_reales_' + db_name
    ...
    if db_exists:
        ...
        for band in bands:
            ...
            if len(db_aux) == 0:
                ...
                #Selecciono los audios para train y para test:
                audios_train = random.sample(audios, k=int(len(audios)*train))
                audios_test = [audio for audio in audios if audio not in audios_train]
                ...
                db = db.append(db_aux, ignore_index=True)
